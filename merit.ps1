...
- Prove on the cloud app URL from create (``/apps/$ConsumerId/play``).
    ...
    $phases = @(
        ...
        @{ n = 8; title = 'Publish UI to merit-prod /apps + optional here.now portal'; script = {
            ...
            if ($hasHere) {
                ...
                Write-Host 'here.now: skipped (no credentials). Portal stub stays in-repo; publish later with merit.ps1 portal.'
            }
        }},
