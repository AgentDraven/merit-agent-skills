...
function Invoke-Create {
    ...
    # Dinner default = platform URL on merit-prod (no builder Vercel). --deploy = own-host.
    # --scaffold-only kept as alias of default platform mode for older docs/CI.
    ...
    $phases = @(
        ...
        @{ n = 8; title = 'Publish UI to merit-prod /apps + optional here.now portal'; script = {
            if (-not $scaffoldOnly) {
        ...
        @{ n = 9; title = 'Success banner'; script = {
            ...
            if ($scaffoldOnly) {
                ...
                if ($script:CreateAppUrl) {
                    Write-Host " Open: $($script:CreateAppUrl)"
                } else {
                    Write-Host " Open: https://merit-prod.vercel.app/apps/$cid/play"
                }
