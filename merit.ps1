...
Commands:
  ...
  closeout --path <repo>   Validate only (verify + git diff --check) ù NOT full MERIT closeout
    ...
    try {
        ...
        if ($branch -eq 'HEAD') {
            if (-not (Test-ArgFlag -ArgList $ArgList -Name '-AllowDetached')) {
                throw 'ship: detached HEAD ù checkout a branch or pass -AllowDetached'
            }
        }
...
function Set-OcCreatorFace {
    ...
    if ($PlayUrl -and $RegisterUrl -and (Test-Path -LiteralPath $portalIndex)) {
        ...
        if ($idx -notmatch [regex]::Escape($RegisterUrl) -and $idx -notmatch 'Join free') {
            $cta = "      <p class=`"oc-ctas`"><a href=`"$PlayUrl`">Open play</a> ù <a href=`"$RegisterUrl`">Join free - Community Member `$0</a></p>"
