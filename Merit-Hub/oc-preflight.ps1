param([Parameter(Mandatory)][string]$DemoRoot)
...
foreach($u in @('https://merit-prod.vercel.app/api/health','https://merit-prod.vercel.app/api/meritsubs/api/v1/health')){try{$r=Invoke-WebRequest -Uri $u -Method Get -TimeoutSec 15;Check "endpoint $u" ($r.StatusCode -eq 200) "HTTP $($r.StatusCode)"}catch{Check "endpoint $u" $false $_.Exception.Message}}
