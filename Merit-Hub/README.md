...
**Current release:** **MERIT Skills `0.5.204`**. The matching Git tag is `skills-v0.5.204`; menu **K** is the advanced rollback selector.
...
- [MERIT CLI student guide](../docs/merit.md)
...
For everyday use, there is one number: **MERIT Skills `0.5.204`**. Hub **2** installs that release by default. Its Git tag is named `skills-v0.5.204`; that is the same release written in Git's tag style, not a second version.
...
2. **Refresh the launcher**
   ...
   - Run it from PowerShell; the corrected release prints `MERIT Skills 0.5.204` first.
...
The normal screen shows only **MERIT Skills `0.5.204`**. It is the launcher release, Hub release, and Hub 2 default payload. The Git tag `skills-v0.5.204` is the same release with the required tag prefix.
